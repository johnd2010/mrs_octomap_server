...
namespace mrs_octomap_server
{
  ...
  void OctomapServer::onInit()
  {
    ...
    for (int i = 0; i < n_sensors_depth_cam_; i++)
    {
      ...
      sh_depth_cam_pc2_.push_back(mrs_lib::SubscribeHandler<sensor_msgs::PointCloud2>(
          shopts, ss.str(), ros::Duration(2.0),
          std::bind(&OctomapServer::callback3dLidarCloud2, this, std::placeholders::_1, DEPTH_CAMERA, i, ss.str(), false)));

      std::stringstream ss2;
      ss2 << "depth_camera_" << i << "_over_max_range_in";

      sh_depth_cam_pc2_.push_back(mrs_lib::SubscribeHandler<sensor_msgs::PointCloud2>(
          shopts, ss2.str(), ros::Duration(2.0),
          std::bind(&OctomapServer::callback3dLidarCloud2, this, std::placeholders::_1, DEPTH_CAMERA, i, ss.str(), true)));
    }
  ...
  void OctomapServer::callbackLaserScan(const sensor_msgs::LaserScan::ConstPtr msg)
  {
    ...
    if (!res)
    {
      ROS_WARN_THROTTLE(1.0, "[OctomapServer]: insertLaserScanCallback(): could not find tf from %s to %s", scan->header.frame_id.c_str(),
                        _world_frame_.c_str());
      return;
    }
                                            ...
                                            const std::string topic, const bool pcl_over_max_range)
  {
    ...
    if (!res)
    {
      ROS_WARN_THROTTLE(1.0, "[OctomapServer]: callback3dLidarCloud2(): could not find tf from %s to %s", cloud->header.frame_id.c_str(),
                        _world_frame_.c_str());
      return;
    }
